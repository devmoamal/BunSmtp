...
export async function relayMail(mail: MailData): Promise<void> {
  ...
  for (const domain of domains) {
    ...
    const { data: mxRecords, error: mxError } = await tryCatch(resolveMx(domain));
    
    if (mxError || !mxRecords || mxRecords.length === 0) {
      logger.error(`MX resolution failed for domain: ${domain}. Reason: ${mxError?.message || 'No MX records found'}`);
      throw new Error(`Domain resolution failure: ${domain}`);
    }
    
    const bestMx = mxRecords.sort((a, b) => a.priority - b.priority)[0];
    if (!bestMx) {
      logger.error(`Priority sorting failed for MX records of domain: ${domain}`);
    ...
    const { error: relayError } = await tryCatch(performManualRelay(host, port, mail, domain));
    
    if (relayError) {
      logger.error(`Protocol synchronization failed during relay to ${host}. Internal error: ${relayError.message}`);
      throw relayError;
    }

    logger.info(`Relay transaction successfully committed for domain: ${domain} via ${host}`);
  }
}
 ...
 */
async function performManualRelay(host: string, port: number, mail: MailData, domain: string): Promise<void> {
        ...
        socket: {
          data(s, data) {
            ...
            if (code >= 400 && currentStep !== 'QUIT') {
              ...
              s.end();
              reject(new Error(`SMTP protocol rejection at step ${currentStep} (Status: ${code}): ${res.trim()}`));
              return;
            }
            ...
            switch (currentStep) {
              ...
              case 'EHLO':
                if (code === 250) {
                  if (hasStartTLS) {
                    logger.debug('Encryption requested: Initiating STARTTLS negotiation.');
                    s.write('STARTTLS\r\n');
          ...
          error(s, err) {
            ...
            if (currentStep !== 'QUIT' && currentStep !== 'COMMIT') {
                reject(new Error(`TCP connection terminated prematurely at step: ${currentStep}`));
            }
          }
        }
      });
